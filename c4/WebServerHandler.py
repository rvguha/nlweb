...
class StreamingHandler(SimpleHTTPRequestHandler):
    ...
    def siteToClass(self, site):
        ...
        if site == "imdb":
            self.logger.debug("Selected GraphStructureHandler for imdb")
            return GraphStructureHandler
        elif site == "bc_product":
            self.logger.debug("Selected ContextSensitiveHandler for backcountry")
            return ContextSensitiveHandler
        elif site == "zillow":
            self.logger.debug("Selected LocationSensitiveHandler for zillow")
            return LocationSensitiveHandler
        elif site == "latam_recipes":
            self.logger.debug("Selected ItemTypeSensitiveRedirectHandler for latam_recipes")
            return ItemTypeSensitiveRedirectHandler
        elif item_type == "Recipe":
            self.logger.debug("Selected RecipeHandler for Recipe item type")
            return RecipeHandler
        elif site == "nlws":
            self.logger.debug("Selected NLWebHandler for NLWeb item type")
            return NLWSHandler
        else:
