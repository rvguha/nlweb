...
class LLMClients:
    ...
    _azure_openai: Optional[AzureOpenAI] = None
    _async_azure_openai: Optional[AsyncAzureOpenAI] = None
    ...
    @classmethod
    def get_azure_openai(cls) -> AzureOpenAI:
        if cls._azure_openai is None:
            cls._azure_openai = AzureOpenAI(
                api_key=get_azure_api_key(),
                azure_endpoint=get_azure_endpoint(),
                api_version=get_azure_api_version()
            )
        return cls._azure_openai
    
    @classmethod
    def get_async_azure_openai(cls) -> AsyncAzureOpenAI:
        if cls._async_azure_openai is None:
            cls._async_azure_openai = AsyncAzureOpenAI(
                api_key=get_azure_api_key(),
                azure_endpoint=get_azure_endpoint(),
                api_version=get_azure_api_version()
            )
        return cls._async_azure_openai
...
def get_azure_endpoint() -> str:
    """Get the Azure endpoint from environment variables."""
    import os
...
def get_azure_api_version() -> str:
    """Get the Azure API version from environment variables or use default."""
    import os
    return os.environ.get("AZURE_OPENAI_API_VERSION", "2024-02-15-preview")
...
def get_provider(model_name: str) -> ModelProvider:
    ...
    elif model_name.startswith(("deepseek-", "azure-deepseek-")):
...
def get_structured_completion(
    ...
    Args:
        ...
        model_name (str): The model name (e.g., "gpt-4", "claude-3-opus-20240229", "gemini-pro", "azure-deepseek-v1")
    ...
    try:
        if provider == ModelProvider.OPENAI:
            ...
            # Use Azure OpenAI client for DeepSeek models
            client = LLMClients.get_azure_openai()
            
            # Extract the deployment name from the model name if it follows azure-deepseek- pattern
            if model_name.startswith("azure-deepseek-"):
                deployment_name = model_name.replace("azure-", "", 1)
            else:
...
async def get_structured_completion_async(
    ...
    try:
        if provider == ModelProvider.OPENAI:
            ...
            # Use Azure OpenAI client for DeepSeek models
            client = LLMClients.get_async_azure_openai()
